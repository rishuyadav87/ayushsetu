          ...
          {rows.map(r => (
            <div key={r.id} className={`bg-white rounded-xl border shadow-sm overflow-hidden ${r.violationCount >= 3 ? 'border-red-300' : r.violationCount ? 'border-amber-200' : 'border-gray-100'}`}>
              <div className="relative bg-gray-900 aspect-video">
                ...
                <div className={`absolute top-2 left-2 text-[10px] font-bold px-1.5 py-0.5 rounded flex items-center gap-1 ${r.online ? 'bg-green-600 text-white' : 'bg-gray-600 text-white'}`}>
              ...
              <div className="p-4">
                <div className="flex justify-between gap-2">
                  ...
                  <div className={`text-sm font-bold whitespace-nowrap ${r.violationCount ? 'text-red-600' : 'text-green-600'}`}>{r.violationCount} ⚠</div>
                </div>
